...
public class WideAnglePanoramaModule
        ...
        TopViewManager.SettingListener,OnShutterButtonListener
        {
    ...
    private static final int MSG_PREVIEW_STARTED = 6;

    private static final int SCREEN_DELAY = 2 * 60 * 1000;

    @SuppressWarnings("unused")
    private static final String TAG = "YUHUAN_WideAnglePanoramaModule";
    ...
    private TctWideAnglePanoramaUI mUI;
    ...
    public MosaicFrameProcessor getMosaicFrameProcessor() {
        return mMosaicFrameProcessor;
    }
     ...
     *
     * change to FOCUS_MODE_CONTINUOUS_VIDEO for new requirement
     * modify by minghui.hua for PR937997
     */
    ...
    private static final int REQUEST_SETGPS = 1002;
    private OrientationManager mOrientationManager;
    private ComboPreferences mPreferences;
    private boolean mMosaicPreviewConfigured;
    private boolean mPreviewFocused = true;

    private boolean mPauseWhenSaving = false; // PR879002-lan.shan-001 Add
    ...
    private int mInitOrientation=-1;

    @Override
    public void init(CameraActivity activity, View parent) {
    	Log.d(TAG,"###YUHUAN###init ");
        mActivity = activity;
        mRootView = parent;

        mOrientationManager = OrientationManager.getInstance();
        mCaptureState = CAPTURE_STATE_VIEWFINDER;
        mUI = new TctWideAnglePanoramaUI(mActivity, this, (ViewGroup) mRootView);
        ...
        mActivity.getCameraViewManager().getTopViewManager().setListener(this);
        mActivity.getCameraViewManager().getShutterManager().setOnShutterButtonListener(this);
        ...
        mPreferences.setLocalId(mActivity, mActivity.getCameraId());
        ...
        Log.d(TAG,"###YUHUAN###init#savePathValue=" + savePathValue);
            ...
            public void handleMessage(Message msg) {
                ...
                case MSG_GENERATE_FINAL_MOSAIC_ERROR:
                    ...
                    if (mPaused) {
                        ...
                        mUI.showAlertDialog(mDialogTitle, mDialogPanoramaFailedString,
                ...
                case MSG_PREVIEW_STARTED: {
                	...
                	Log.d(TAG,"###YUHUAN###init#getAction=" + mActivity.getCurIntent().getAction());
    ...
    public void onSharedPreferenceChanged() {
        UpdateCameraParametersPreference();//[BUG-898800] set the anti-banding lijuan.zhang add ,20150114
        // updateGridManager();
        /*[BUGFIX]added by lijuan.zhang ,2015-05-28,PR992513/991647 begin*/
        String savePathValue = mPreferences.getString(CameraSettings.KEY_CAMERA_SAVEPATH, "-1");
        Storage.setSaveSDCard(savePathValue.equals("1"), savePathValue.equals("-1"));
        ...
        CameraActivity.TraceLog(TAG, "onSharedPreferenceChanged");
       ...
       boolean recordLocation = RecordLocationPreference.get(mPreferences,
                 mContentResolver);
       CameraActivity.TraceLog(TAG, "onSharedPreferenceChanged : mPreferences = " + mPreferences +
                "; mContentResolver = " + mContentResolver + "; recordLocation = " + recordLocation);
    ...
    // according to "PhotoModule" for PR1010577
    private boolean checkLocationService(boolean enable) {
        boolean recordLocation = enable;
        // If GPS tag is set off, not check needed.
        CameraActivity.TraceLog(TAG, "checkLocationService() : recordLocation = " + recordLocation);
        if (!recordLocation) return recordLocation;

        boolean gpsEnabled = Settings.Secure.isLocationProviderEnabled(
                mContentResolver, android.location.LocationManager.GPS_PROVIDER);
        boolean networkEnabled = Settings.Secure.isLocationProviderEnabled(
                mContentResolver, android.location.LocationManager.NETWORK_PROVIDER);
        CameraActivity.TraceLog(TAG, "checkLocationService : gpsEnabled = " + gpsEnabled +
        ...
        if (!(gpsEnabled || networkEnabled)) {
            ...
            new AlertDialog.Builder(mActivity)
                    ...
                    .setPositiveButton(R.string.location_service__dialog_setting,
                            new DialogInterface.OnClickListener() {
                                @Override
                                public void onClick(DialogInterface dialog, int arg1) {
                                    Intent intent = new Intent(
                                            "android.settings.LOCATION_SOURCE_SETTINGS");
                                    mActivity.startActivityForResult(intent, REQUEST_SETGPS);
                                }
                            })
            ...
            mActivity.getCameraViewManager().getTopViewManager().refreshMenu();
        }
    ...
    private void setGpsTagPreference(String value) {
        CameraActivity.TraceLog(TAG, "setGpsTagPreference : value = " + value);
    ...
    private void loadCameraPreferences() {
        ...
        //        mActivity.getCameraId(), CameraHolder.instance().getCameraInfo());
    ...
    private boolean openCamera() {
        ...
        int cameraId = mActivity.getCameraId();
    ...
    private void setupCaptureParams(Parameters parameters) {
        ...
        Log.d(TAG, "###YUHUAN###setupCaptureParams#mCameraPreviewWidth=" + mCameraPreviewWidth);
        Log.d(TAG, "###YUHUAN###setupCaptureParams#mCameraPreviewHeight=" + mCameraPreviewHeight);
        ...
        Log.d(TAG, "###YUHUAN###setupCaptureParams#mHorizontalViewAngle=" + mHorizontalViewAngle);
        mVerticalViewAngle =  parameters.getVerticalViewAngle();
        Log.d(TAG, "###YUHUAN###setupCaptureParams#mVerticalViewAngle=" + mVerticalViewAngle);
    }
    ...
    public int getPreviewBufSize() {
        ...
        Log.d(TAG,"###YUHUAN###getPreviewBufSize#pixelInfo.bitsPerPixel=" + pixelInfo.bitsPerPixel);
    ...
    /*[BUG-898800] set the anti-banding lijuan.zhang add ,20150114 begin*/
    ...
    /*[BUG-898800] set the anti-banding lijuan.zhang add ,20150114 end*/

    // modify by minghui.hua for PR971928,PR970566
...
//        }
    ...
    private void configMosaicPreview() {
        if (mPreviewUIWidth == 0 || mPreviewUIHeight == 0
                || mUI.getSurfaceTexture() == null || mThreadRunning) { // PR879002-lan.shan-001 Modify
            return;
        }
        ...
        Log.d(TAG,"###YUHUAN###configMosaicPreview#isLandscape=" + isLandscape);
    ...
    public void startCapture() {
        ...
        mUI.onStartCapture();
        mUI.setStatues(true); //Add by fei.gao for PR953178 
        mMosaicFrameProcessor.setProgressListener(new MosaicFrameProcessor.ProgressListener() {
            @Override
            public void onProgress(boolean isFinished, float panningRateX, float panningRateY,
                    float progressX, float progressY) {
                float accumulatedHorizontalAngle = progressX * mHorizontalViewAngle;
                Log.d(TAG,"###YUHUAN###startCapture#accumulatedHorizontalAngle=" + accumulatedHorizontalAngle);
                
                float accumulatedVerticalAngle = progressY * mVerticalViewAngle;
                Log.d(TAG,"###YUHUAN###startCapture#accumulatedVerticalAngle=" + accumulatedVerticalAngle);
                Log.d(TAG,"###YUHUAN###startCapture#isFinished=" + isFinished);
                if (isFinished
        ...
        //[BUGFIX]-Delete by TCTNB,haibo.chen, 2014-08-25,PR764218 Begin
//        mOrientationManager.lockOrientation();
        //[BUGFIX]-Delete by TCTNB,haibo.chen, 2014-08-25,PR764218 End
    ...
    public void stopCapture(boolean aborted) {
        mCaptureState = CAPTURE_STATE_VIEWFINDER;
        mUI.onStopCapture();
        mUI.setStatues(false);//Add by fei.gao for PR953178 	
        mMosaicFrameProcessor.setProgressListener(null);
        stopCameraPreview();

        mCameraTexture.setOnFrameAvailableListener(null);

        Log.d(TAG,"###YUHUAN###stopCapture#aborted=" + aborted);
        Log.d(TAG,"###YUHUAN###stopCapture#mThreadRunning=" + mThreadRunning);
        ...
        if(aborted){//PR1000919-sichao.hu added, to handle cases when the saving is aborted
            mMainHandler.sendMessage(mMainHandler.obtainMessage(
                    MSG_END_DIALOG_RESET_TO_PREVIEW));
        }
        keepScreenOnAwhile();
...
//        boolean isCaptureSoundEnable = mPreferences.getString(CameraSettings.KEY_SOUND,
//                mActivity.getString(R.string.pref_camera_capturesound_default))
//               .equals(mActivity.getString(R.string.setting_on_value));
        String defaultValue = CustomUtil.getInstance().getBoolean(CustomFields.DEF_TCTCAMERA_SHUTTER_SOUND_ON, true) ? "on" : "off";
        boolean isCaptureSoundEnable = mPreferences.getString(CameraSettings.KEY_SOUND,
        		defaultValue)
               .equals(mActivity.getString(R.string.setting_on_value));
        /*[FEATURE]-Set the perso of shutter sound added by lijuan.zhang ,20141103 end*/
        
        Log.d(TAG,"###YUHUAN###onShutterButtonClick#mCaptureState=" + mCaptureState);
    ...
    public void saveHighResMosaic() {
    	Log.d(TAG,"###YUHUAN###saveHighResMosaic into ");
    	mActivity.disableDrawer();//PR900624-sichao.hu added
        runBackgroundThread(new Thread() {
            @Override
            public void run() {
    ...
    private void onBackgroundThreadFinished() {
    	Log.d(TAG,"###YUHUAN###onBackgroundThreadFinished");
    	mActivity.enableDrawer();//PR900624-sichao.hu added
    ...
    private void reset() {
        mPauseWhenSaving = false; // PR879002-lan.shan-001 Add
        mCaptureState = CAPTURE_STATE_VIEWFINDER;
        //[BUGFIX]-Delete by TCTNB,haibo.chen, 2014-08-25,PR764218 Begin
//        mOrientationManager.unlockOrientation();
        ...
        mActivity.enableDrawer();//Add by fei.gao for PR996397
        ...
        Log.d(TAG,"###YUHUAN###reset#mPreviewFocused=" + mPreviewFocused);
        if (mPreviewFocused) {
            mUI.showPreviewUI();
        }
    ...
    private void resetToPreviewIfPossible() {
        ...
        Log.d(TAG,"###YUHUAN###resetToPreviewIfPossible#mMosaicFrameProcessorInitialized=" + mMosaicFrameProcessorInitialized);
        Log.d(TAG,"###YUHUAN###resetToPreviewIfPossible#mUI.getSurfaceTexture()=" + mUI.getSurfaceTexture());
        Log.d(TAG,"###YUHUAN###resetToPreviewIfPossible#mMosaicPreviewConfigured=" + mMosaicPreviewConfigured);
        ...
        Log.d(TAG,"###YUHUAN###resetToPreviewIfPossible#mPaused=" + mPaused);
        if (!mPaused) {
            startCameraPreview();
        }
    }
	...
	private void clearMosaicFrameProcessorIfNeeded() {
		Log.d(TAG, "###YUHUAN###clearMosaicFrameProcessorIfNeeded#mPaused=" + mPaused);
		Log.d(TAG,"###YUHUAN###clearMosaicFrameProcessorIfNeeded#mThreadRunning=" + mThreadRunning);
		...
		Log.d(TAG,"###YUHUAN###clearMosaicFrameProcessorIfNeeded#mMosaicFrameProcessorInitialized="
    ...
    private void initMosaicFrameProcessorIfNeeded() {
        ...
        Log.d(TAG,"###YUHUAN###initMosaicFrameProcessorIfNeeded#mCameraPreviewWidth=" + mCameraPreviewWidth);
        Log.d(TAG,"###YUHUAN###initMosaicFrameProcessorIfNeeded#mCameraPreviewHeight=" + mCameraPreviewHeight);
...
//        mOrientationManager.pause();
    ...
    public void onPauseAfterSuper() {
        ...
        if (mUI.isSavingProgressRunning()) {
            mPauseWhenSaving = true;
        } else {
            mUI.showPreviewCover();
        }
        // PR879002-lan.shan-001 Add end
        //mUI.showPreviewCover(); // PR870395-lan.shan-001 Remove
        releaseCamera();
        synchronized (mRendererLock) {
            mCameraTexture = null;
    ...
    public void onOrientationChanged(int orientation) {
    	...
    	if(mActivity.isReversibleEnabled()){
    		if(mInitOrientation==-2){
            	mInitOrientation=mOrientation;
            }
            if(mInitOrientation==-1){
            	mInitOrientation=-2;
            }
 ...
 // The display rotation in degrees. This is only valid when mCameraState is
    ...
    public void onResumeAfterSuper() {
    	Log.d(TAG,"###YUHUAN###onResumeAfterSuper ");
        ...
        Log.d(TAG,"###YUHUAN###onResumeAfterSuper#mThreadRunning= " + mThreadRunning);
...
//        mOrientationManager.resume();
        ...
        boolean recordLocation = RecordLocationPreference.get(mPreferences,
                mContentResolver);
        Log.d(TAG,"###YUHUAN###onResumeAfterSuper#recordLocation= " + recordLocation);
        mLocationManager.recordLocation(recordLocation);
    ...
    public MosaicJpeg generateFinalMosaic(boolean highRes) {
        ...
        Log.d(TAG,"###YUHUAN###generateFinalMosaic#mosaicReturnCode=" + mosaicReturnCode);
    ...
    private void startCameraPreview() {
    	Log.d(TAG,"###YUHUAN###startCameraPreview ");
        if (mCameraDevice == null) {
...
//            }
        ...
        mMainHandler.sendEmptyMessage(MSG_PREVIEW_STARTED);
...
//        mActivity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
    ...
    public void cancelHighResStitching() {
        if (mPaused || ((mCameraTexture == null) && !mPauseWhenSaving)) return; // PR879002-lan.shan-001 Modify
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_SETGPS) {
            boolean gpsEnabled = Settings.Secure.isLocationProviderEnabled(
                    mContentResolver, android.location.LocationManager.GPS_PROVIDER);
            boolean networkEnabled = Settings.Secure.isLocationProviderEnabled(
                    mContentResolver, android.location.LocationManager.NETWORK_PROVIDER);
            CameraActivity.TraceLog(TAG, "onActivityResult : gpsEnabled = " + gpsEnabled
                    + " ; networkEnabled = " + networkEnabled);
            if (!(gpsEnabled || networkEnabled)) {
                if (isLocationOpened) {
                    isLocationOpened = false;
            ...
            boolean recordLocation = RecordLocationPreference.get(mPreferences,
                    mContentResolver);
            CameraActivity.TraceLog(TAG,"onActivityResult->setLocationPreference(VALUE_ON) " +
    ...
    public ComboPreferences getPreferences() {
        Log.i(TAG,"LMCH04142 mPreferences:"+mPreferences);
        return mPreferences; //wxt add, maybe need add something  later
    }
    ...
    public Bitmap getGenerateMosaicTempBitmap() {
        int mosaicReturnCode = mMosaicFrameProcessor.createMosaic(false);
        if (mosaicReturnCode == Mosaic.MOSAIC_RET_CANCELLED
            || mosaicReturnCode == Mosaic.MOSAIC_RET_ERROR) {
             Log.e(TAG,"createMosaic returned null.");
            return null;
        }

        int[] imageData = mMosaicFrameProcessor.getFinalMosaic();
        if (imageData == null || imageData.length <= 0) {
            Log.e(TAG, "getFinalMosaic() returned null.");
            return null;
        }
        int len = imageData.length - 2;
        int width =imageData[len + 0];
        int height =imageData[len + 1];
        Log.v(TAG, "getGenerateMosaicTempBitmap ImLength = " + (len) + ", W = " + width + ", H = " + height);

        Bitmap b= null;
        ByteArrayOutputStream out = null;
        MosaicJpeg jpeg = null;
        try{
            b=android.graphics.Bitmap.createBitmap(imageData, width, height, android.graphics.Bitmap.Config.RGB_565);
            out = new ByteArrayOutputStream();
            b.compress(Bitmap.CompressFormat.JPEG, 80, out);
            jpeg = new MosaicJpeg(out.toByteArray(), width, height);
        }catch(Exception e){
            return null;
        }finally{
            try{
        ...
        byte[] jdata = jpeg.data;
        Bitmap temp = null;
        if (jdata != null) {
           temp = BitmapFactory.decodeByteArray(jdata, 0,
                        jdata.length);
        }
        return temp;
    }
