...
public class CameraSettings {
    ...
    public static final String KEY_TOUCH_SHUTTER = "pref_camera_touch_shutter_key";
    public static final String KEY_CONTINUOUS_NUM = "pref_camera_continuous_num_key";
    public static final String KEY_CAPTURE_MODE = "pref_camera_cap_mode_key";
    public static final String KEY_NORMAL_CAPTURE_KEY = "pref_camera_normal_capture_key";
    public static final String KEY_BACK = "pref_camera_back_key";

    public static final String KEY_RESTORE_TO_DEFAULT = "pref_camera_restore_to_default";
    public static final String KEY_GPS_TAG = "pref_camera_gps_tag_key";
    public static final String KEY_ADVANCED_SETTING = "pref_camera_advanced_setting_key";
    public static final String KEY_CAMERA_NIGHT = "pref_camera_night_key";
    public static final String KEY_CAMERA_SPORTS = "pref_camera_sports_key";
    public static final String KEY_SOUND_RECORDING = "pref_camera_soundrecording_key";
    public static final String KEY_VIDEO_EIS = "pref_video_eis_key";
    public static final String KEY_VIDEO_FRONT_QUALITY = "pref_video_front_quality_key";
    //[FEATURE]-ADD-END by TCTNB(Mingchun.Li)
    public static final String KEY_VOICE_SHUTTER = "pref_camera_voice_shutter_key";//[FEATURE]-Add by TCTNB.(Liu Jie),12/18/2013,550519,VoiceShooting
    public static final String KEY_VIDEO_LIMIT_FOR_MMS_KEY = "pref_video_limit_for_mms_key";
    public static final String KEY_VIDEO_QUALITY = "pref_video_quality_key";
    public static final String KEY_VIDEO_TIME_LAPSE_FRAME_INTERVAL = "pref_video_time_lapse_frame_interval_key";
    public static final String KEY_PICTURE_SIZE = "pref_camera_picturesize_key";
    public static final String KEY_PICTURE_SIZE_FLAG = "pref_camera_picturesize_flag_key";
    ...
    public static final String KEY_AUTOEXPOSURE_LOCK = "pref_camera_auto_exposure_lock";
    public static final String KEY_CAMERA_WAV_DENOISE = "pref_camera_wav_denoise";
    ...
    public static final String KEY_REFOCUS_ON_SHOOT = "pref_camera_refocusonshoot_key";
    public static final String KEY_CAMERA_FFF="pref_camera_fast_face_focus";
    //public static final String KEY_CAMERA_ARCSOFT_NIGHT="pref_camera_arcsoftnight_key";
    //public static final String KEY_CAMERA_ARCSOFT_ASD="pref_camera_arcsoftasd_key";
    
    //public static final String KEY_CAMERA_ARCSOFT_ASD_NIGHT_MODE="pref_camera_arcsoftasd_night_key";
    //public static final String KEY_CAMERA_ARCSOFT_ASD_BACK_NIGHT_MODE="pref_camera_arcsoftasd_back_night_key";
    
    public static final String KEY_CAMERA_GRID="pref_camera_grid_key";
    //public static final String KEY_ARCSOFT_SOFTEN="pref_camera_arcsoften_key";
    //public static final String KEY_ARCSOFT_BRIGHT="pref_camera_arcbright_key";
    ...
    public static final String NIGHT_MODE_TOAST_ISFIRST_DISPLAY="pre_night_mode_is_first_display";
    public static final String BACK_LIGHt_TOAST_ISFIRST_DISPLAY="pre_back_light_is_first_display";
    ...
    private static final String TAG = "YUHUAN_CameraSettings";
    ...
    public static void initialCameraPictureSize(
        ...
        Log.e("TAG", "###YUHUAN###initialCameraPictureSize");
        Exception e = new Exception();
        e.printStackTrace();
        List<Size> supported = parameters.getSupportedPictureSizes();
        if (supported == null) return;
        
        // yuhuan 20150703 for picture size test  ===begin
        for(int i=0;i<supported.size();i++) {
        	Size mSize = supported.get(i);
        	Log.d(TAG,"###YUHUAN###initialCameraPictureSize#supported[" + i + "]="+ mSize.width + "x" + mSize.height);
        }
        // yuhuan 20150703 for picture size test  ===end

        for (String candidate : context.getResources().getStringArray(
                R.array.pref_camera_picturesize_entryvalues)) {
            if (!isSuit2TheTargetRatio(context,candidate))continue;
    ...
    private static boolean isSuit2TheTargetRatio(Context context,String candidate) {
            CameraActivity mCameraActivity = (CameraActivity) context;
            int index = candidate.indexOf('x');
            if (index == NOT_FOUND) return false;
            int width = Integer.parseInt(candidate.substring(0, index));
            Log.d(TAG,"###YUHUAN###isSuit2TheTargetRatio#width=" + width);
            int height = Integer.parseInt(candidate.substring(index + 1));
            Log.d(TAG,"###YUHUAN###isSuit2TheTargetRatio#height=" + height);
            String currentRatio = getRatioString(width,height);
            Log.d(TAG,"###YUHUAN###isSuit2TheTargetRatio#currentRatio=" + currentRatio);
            
            if (currentRatio != null &&
                currentRatio.equals(mCameraActivity.getGC().getPreviewRatio()))
                return true;
            return false;
    }

    public static String getSupportedRawFormat(Parameters mPhotoParameters) {
        if (mPhotoParameters == null) {
            return null;
        }

        String str = mPhotoParameters.get(KEY_QC_PICTURE_FORMAT);
        ...
        if (size > 0) {
            for (int i=0;i<size;i++) {
                String picform = supportedList.get(i);
                int len = picform.length();
                if (len > 10) {
                    String subPicform = picform.substring(0, 10);
                    if (subPicform.equals("bayer-qcom"))
                        return picform;
                }
            }
        ...
        if (size == 0) {
            ...
            Log.i(TAG,"get from param rawmode:"+supportedList.get(1));
            return supportedList.get(1);
        }
    }
    ...
    public static boolean setCameraPictureSize(
        ...
        if (index == NOT_FOUND) return false;
        int width = Integer.parseInt(candidate.substring(0, index));
        Log.d(TAG,"###YUHUAN###setCameraPictureSize#width=" + width);
        int height = Integer.parseInt(candidate.substring(index + 1));
        Log.d(TAG,"###YUHUAN###setCameraPictureSize#height=" + height);
   ...
   public static List<String> getSupportedFlipMode(Parameters params){
    ...
    private void initPreference(PreferenceGroup group) {
        ...
        ListPreference videoFrontQuality = group.findPreference(KEY_VIDEO_FRONT_QUALITY);
        ...
        //wxt no need now IconListPreference exposure = (IconListPreference) group.findPreference(KEY_EXPOSURE);
        ...
        //}
          ...
          //pr1005366-yuguan.chen removed begin for video quality perso
//        if (videoQuality != null && GC.CAMERAID_BACK== mCameraId) {
//            filterUnsupportedOptions(group, videoQuality, getSupportedVideoQuality(
//                   mCameraId,mParameters));
//        }
//
//        if (videoFrontQuality != null && GC.CAMERAID_FRONT== mCameraId) {
//            filterUnsupportedOptions(group, videoFrontQuality, getSupportedVideoQuality(
//                   mCameraId,mParameters));
//        }
          //pr1005366-yuguan.chen removed end for video quality perso
        ...
        //wxt no need now if (exposure != null) buildExposureCompensation(group, exposure);
        ...
        if (mContext instanceof CameraActivity)
            ((CameraActivity)mContext).getGC().initPreference(group);
    }
...
//           }
//        }
        if (CamcorderProfile.hasProfile(cameraId, CamcorderProfileEx.getQuality(cameraId, CamcorderProfile.QUALITY_1080P))) {
           if (checkSupportedVideoQuality(parameters,1920,1080)){
              supported.add(Integer.toString(CamcorderProfile.QUALITY_1080P));
           }else if(checkSupportedVideoQuality(parameters,1920,1088)){
              supported.add(Integer.toString(CamcorderProfile.QUALITY_1080P));
           }
        }
        if (CamcorderProfile.hasProfile(cameraId, CamcorderProfileEx.getQuality(cameraId, CamcorderProfile.QUALITY_720P))) {
           if (checkSupportedVideoQuality(parameters,1280,720)){
              supported.add(Integer.toString(CamcorderProfile.QUALITY_720P));
           }
        }
        if (CamcorderProfile.hasProfile(cameraId, CamcorderProfileEx.getQuality(cameraId, CamcorderProfile.QUALITY_480P))) {
           if (checkSupportedVideoQuality(parameters,720,480)){
              supported.add(Integer.toString(CamcorderProfile.QUALITY_480P));
           }else if(checkSupportedVideoQuality(parameters,640,480)){
               supported.add(Integer.toString(CamcorderProfile.QUALITY_480P));
           }
        }
...
//           }
//        }
        if (CamcorderProfile.hasProfile(cameraId, CamcorderProfileEx.getQuality(cameraId,10))) {//VGA
           //if (checkSupportedVideoQuality(parameters,640,480)){
              supported.add(Integer.toString(10));
           //}
        }
        if (CamcorderProfile.hasProfile(cameraId, CamcorderProfileEx.getQuality(cameraId,CamcorderProfile.QUALITY_CIF))) {
