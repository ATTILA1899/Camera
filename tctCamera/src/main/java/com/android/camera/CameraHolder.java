    ...
    public synchronized CameraProxy open(
        ...
        if (DEBUG_OPEN_RELEASE) {
            ...
            if (mCameraOpened) {
                ...
                dumpStates();
                return null;//PR973735 , avoid double open camera
            }
        }
        ...
        if (mCameraDevice == null) {
            ...
            PictureSizePerso picturePerso=PictureSizePerso.getInstance();
            if(!picturePerso.isInitialized(cameraId)){
            	picturePerso.init(mCameraDevice, cameraId);
            }
            //pr1005366-yuguan.chen added begin for video quality perso
            VideoSizePerso videoPerso=VideoSizePerso.getInstance();
            if(!videoPerso.isInitialized(cameraId)){
            	videoPerso.init(mCameraDevice, cameraId);
            }
            //pr1005366-yuguan.chen added end for video quality perso
            
            List<PictureSize> sizes=picturePerso.getPersoSupportedSizes(cameraId);
