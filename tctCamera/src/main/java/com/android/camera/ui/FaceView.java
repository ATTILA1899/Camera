...
public class FaceView extends View
    ...
    private final boolean SQUARE_FACE_INDICATOR = true;
    ...
    public RectF [] getFaceRectF(Face [] faces){
        if (faces != null && faces.length > 0) {
            RectF [] rectF = new RectF[faces.length];
            mMatrix.postRotate(mOrientation);
            for (int i = 0; i < faces.length; i++) {
                rectF[i]=new RectF(faces[i].rect);
                mMatrix.mapRect(rectF[i]);
            }
            return rectF;
        }
        return null;
    ...
    public void setFaces(Face[] faces) {
        Log.w(TAG, "Num of faces=" + faces.length);
        if (mPause) return;
        if (mFaces != null) {
...
//        invalidate();
    ...
    protected void drawSquare(Canvas canvas) {
        ...
        for (int i = 0; i < mFaces.length; i++) {
           ...
           if (LOGV) {
                CameraUtil.dumpRect(mRect, "FaceView Original rect");
           }
            mMatrix.mapRect(mRect);
            if (LOGV) {
                CameraUtil.dumpRect(mRect, "FaceView Transformed rect");
            }
            //Log.e(TAG, "drawSquare 00000  left:"+Math.round(mRect.left) +" top:"+Math.round(mRect.top)+" right:"+Math.round(mRect.right) +" bottom:"+Math.round(mRect.bottom));
