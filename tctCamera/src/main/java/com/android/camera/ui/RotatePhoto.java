 ...
 */
public class RotatePhoto extends TwoStateImageView implements Rotatable {

    @SuppressWarnings("unused")
    private static final String TAG = "RotatePhoto";
    ...
    public RotatePhoto(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public RotatePhoto(Context context) {
        super(context);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // Scale down the image first if required.
        	if(mCurrentDegree==0 || mCurrentDegree ==180){
        			float ratio = Math.min((float) width / w, (float) height / h);
        			canvas.scale(ratio, ratio, width / 2.0f, height / 2.0f);	
        	}else{
        			float ratio = Math.min((float) width / h, (float) height / w);
        			canvas.scale(ratio, ratio, width / 2.0f, height / 2.0f);	
        	}
        
        canvas.translate(left + width / 2f, top + height / 2f); // PR837082-lan.shan-001 Modify
        canvas.rotate(-mCurrentDegree);
        canvas.translate(-w / 2f, -h / 2f); // PR837082-lan.shan-001 Modify
