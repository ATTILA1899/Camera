...
class AndroidCameraManagerImpl implements CameraManager {
    private static final String TAG = "YUHUAN_" +
    ...
    private class CameraHandler extends Handler {
        ...
        public void handleMessage(final Message msg) {
            try {
                switch (msg.what) {
                    case OPEN_CAMERA:
                    	CameraActivity.TraceLog(TAG, "Open Camera");
                        ...
                        CameraActivity.TraceLog(TAG, "Open Camera end");
                        return;

                    case RELEASE:
                    	CameraActivity.TraceLog(TAG, "Release Camera");
                        mCamera.release();
                        mCamera = null;
                        CameraActivity.TraceLog(TAG, "Release Camera end");
                        return;

                    case RECONNECT:
                    	CameraActivity.TraceLog(TAG, "Reconnect Camera");
                        ...
                        CameraActivity.TraceLog(TAG, "Reconnect Camera end");
                        return;

                    case UNLOCK:
                    	CameraActivity.TraceLog(TAG, "Unlock Camera");
                        mCamera.unlock();
                        CameraActivity.TraceLog(TAG, "Unlock Camera end");
                        return;

                    case LOCK:
                    	CameraActivity.TraceLog(TAG, "Lock Camera");
                        mCamera.lock();
                        CameraActivity.TraceLog(TAG, "Lock Camera end");
                        return;

                    case SET_PREVIEW_TEXTURE_ASYNC:
                    	CameraActivity.TraceLog(TAG, "Set preview texture async");
                        setPreviewTexture(msg.obj);
                        CameraActivity.TraceLog(TAG, "Set preview texture async end");
                        return;

                    case SET_PREVIEW_DISPLAY_ASYNC:
                    	CameraActivity.TraceLog(TAG, "Set preview display async");
                        try {
                            mCamera.setPreviewDisplay((SurfaceHolder) msg.obj);
                        } catch (Exception e) {
                        	CameraActivity.TraceLog(TAG, "Set preview display async failed");
                            throw new RuntimeException(e);
                        }
                        CameraActivity.TraceLog(TAG, "Set preview display async success");
                        return;

                    case START_PREVIEW_ASYNC:
                    	CameraActivity.TraceLog(TAG,"Start Preview");
                        mCamera.startPreview();
                        CameraActivity.TraceLog(TAG,"Start Preview end");
                        return;

                    case STOP_PREVIEW:
                    	CameraActivity.TraceLog(TAG,"Stop preview");
                        mCamera.stopPreview();
                        CameraActivity.TraceLog(TAG,"Stop preview end");
                        return;

                    case SET_PREVIEW_CALLBACK_WITH_BUFFER:
                    	CameraActivity.TraceLog(TAG,"set preview callback with buffer");
                        mCamera.setPreviewCallbackWithBuffer(
                            (PreviewCallback) msg.obj);
                        CameraActivity.TraceLog(TAG,"set preview callback with buffer end");
                        return;

                    case SET_ONE_SHOT_PREVIEW_CALLBACK:
                    	CameraActivity.TraceLog(TAG,"set one shot preview callback");
                        if (mCamera != null) {
                            mCamera.setOneShotPreviewCallback((PreviewCallback) msg.obj);
                        }
                        CameraActivity.TraceLog(TAG,"set one shot preview callback end");
                        break;
                    ...
                    case AUTO_FOCUS:
                    	CameraActivity.TraceLog(TAG,"auto focus");
                        mCamera.autoFocus((AutoFocusCallback) msg.obj);
                        CameraActivity.TraceLog(TAG,"auto focus end" );
                        return;

                    case CANCEL_AUTO_FOCUS:
                    	CameraActivity.TraceLog(TAG,"cancel auto focus");
                        mCamera.cancelAutoFocus();
                        CameraActivity.TraceLog(TAG,"cancel auto focus end");
                        return;

                    case SET_AUTO_FOCUS_MOVE_CALLBACK:
                    	CameraActivity.TraceLog(TAG,"set auto focus move callback");
                        setAutoFocusMoveCallback(mCamera, msg.obj);
                        CameraActivity.TraceLog(TAG,"set auto focus move callback end");
                        return;

                    case SET_DISPLAY_ORIENTATION:
                    	CameraActivity.TraceLog(TAG,"set display orientation");
                        mCamera.setDisplayOrientation(msg.arg1);
                        CameraActivity.TraceLog(TAG,"set display orientation end");
                        return;

                    case SET_ZOOM_CHANGE_LISTENER:
                    	CameraActivity.TraceLog(TAG,"set zoom change listener");
                        mCamera.setZoomChangeListener(
                            (OnZoomChangeListener) msg.obj);
                        CameraActivity.TraceLog(TAG,"set zoom change listener end");
                        return;

                    case SET_FACE_DETECTION_LISTENER:
                    	CameraActivity.TraceLog(TAG,"set face detection listener");
                        setFaceDetectionListener((FaceDetectionListener) msg.obj);
                        CameraActivity.TraceLog(TAG,"set face detection listener end");
                        return;

                    case START_FACE_DETECTION:
                    	CameraActivity.TraceLog(TAG,"start face detection");
                        startFaceDetection();
                        CameraActivity.TraceLog(TAG,"start face detection end");
                        return;

                    case STOP_FACE_DETECTION:
                    	CameraActivity.TraceLog(TAG,"stop face detection");
                        stopFaceDetection();
                        CameraActivity.TraceLog(TAG,"stop face detection end");
                        return;

                    case SET_ERROR_CALLBACK:
                    	CameraActivity.TraceLog(TAG,"set error callback");
                        mCamera.setErrorCallback((ErrorCallback) msg.obj);
                        CameraActivity.TraceLog(TAG,"set error callback end");
                        return;

                    case SET_PARAMETERS:
                    	CameraActivity.TraceLog(TAG,"set Parameters");
                        mParametersIsDirty = true;
                        mParamsToSet.unflatten((String) msg.obj);
                        try{//PR910547-sichao.hu added
                        	mCamera.setParameters(mParamsToSet);
                        }catch(Exception e){
                    		Log.w(TAG, "Set parameter failed");//This could happen under some endurance test
                    	}
                        Log.w(TAG, "set parameter success");
                        return;

                    case GET_PARAMETERS:
                    	CameraActivity.TraceLog(TAG,"Get parameters");
                        if (mParametersIsDirty) {
                        	try{//PR910547-sichao.hu added
                        		mParameters = mCamera.getParameters();
                        	}catch(Exception e){
                        		CameraActivity.TraceLog(TAG, "Get parameter failed");//This could happen under some endurance test
                        	}
                            mParametersIsDirty = false;
                        }
                        CameraActivity.TraceLog(TAG,"Get parameters success");
                        return;

                    case SET_PREVIEW_CALLBACK:
                    	CameraActivity.TraceLog(TAG,"Set preview callback");
                        mCamera.setPreviewCallback((PreviewCallback) msg.obj);
                        CameraActivity.TraceLog(TAG,"Set preview callback end");
                        return;

                    case ENABLE_SHUTTER_SOUND:
                    	CameraActivity.TraceLog(TAG,"enable shutter sound ");
                        enableShutterSound((msg.arg1 == 1) ? true : false);
                        CameraActivity.TraceLog(TAG,"enable shutter sound end");
                        return;

                    case REFRESH_PARAMETERS:
                    	CameraActivity.TraceLog(TAG,"refresh parameters");
                        mParametersIsDirty = true;
                        CameraActivity.TraceLog(TAG,"refresh parameters end");
                        return;
...
//                        mCamera.sendHistogramData();
                    ...
                    case SET_LONGSHOT:
                    	CameraActivity.TraceLog(TAG,"set longshot");
//                        mCamera.setLongshot((Boolean) msg.obj);
                        ExtendCamera extCamera = ExtendCamera.getInstance(mCamera);
                        extCamera.setLongshot((Boolean) msg.obj,mParameters);
                        CameraActivity.TraceLog(TAG,"set longshot end");
                        break;
    ...
    public class AndroidCameraProxyImpl implements CameraManager.CameraProxy {
      ...
      //pr1015398-yuguan.chen add begin
        @Override
        public void cancelAutoFocusSync() {
            mCameraHandler.removeMessages(AUTO_FOCUS);
            mCameraHandler.sendEmptyMessage(CANCEL_AUTO_FOCUS);
            mCameraHandler.waitDone();
        }
      //pr1015398-yuguan.chen add end
        ...
        public void setParameters(Parameters params) {
            ...
            mCameraHandler.obtainMessage(SET_PARAMETERS, flatten)
                    .sendToTarget();
        }
