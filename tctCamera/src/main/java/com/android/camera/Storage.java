...
public class Storage {
    ...
    public static String RAW_DIRECTORY = DCIM + "/Camera/raw";
    ...
    public static void setSaveSDCard(boolean saveSDCard, boolean isFollowSystem) {
        boolean isDefaultStorage = SDCard.instance().isDefaultStorage();
        if (isFollowSystem) {
            sSaveSDCard = isDefaultStorage;
        } else {
            sSaveSDCard = saveSDCard;
        }
        setDCIMPath(isDefaultStorage);
    }
    /// }

    //modify by minghui.hua for PR943681
    private static void setDCIMPath(boolean isSDCardDefaultStorage){
    	Log.d(TAG,"###YUHUAN###setDCIMPath#isSDCardDefaultStorage=" + isSDCardDefaultStorage);
        if(isSDCardDefaultStorage) {
            DCIM = "/storage/sdcard1/DCIM/";
        }else{
    ...
    public static boolean writeFile(String path, byte[] jpeg, ExifInterface exif,
            String mimeType) {
	...
	public static Uri addImage(ContentResolver resolver, String title,
		...
		Log.d(TAG, "###YUHUAN###addImage#title=" + title);
		Log.d(TAG, "###YUHUAN###addImage#orientation=" + orientation);
		Log.d(TAG, "###YUHUAN###addImage#width=" + width);
		Log.d(TAG, "###YUHUAN###addImage#height=" + height);
		Log.d(TAG, "###YUHUAN###addImage#mimeType=" + mimeType);

		String path = generateFilepath(title, mimeType);
		Log.d(TAG, "###YUHUAN###addImage#path=" + path);
		if (writeFile(path, jpeg, exif, mimeType)) {
			// [BUGFIX]-MOD by TCTNB,tao-li, 2014-07-21,PR735438 Begin
			int jpegSize = jpeg.length;
			try {
				jpegSize = (int) ((new File(path)).length());
			} catch (Exception e) {
				Log.e(TAG, "AddImage failed to get file length", e);
			}
			// return addImage(resolver, title, date, location, orientation,
			// jpeg.length, path, width, height, mimeType);
			return addImage(resolver, title, date, location, orientation,
					jpegSize, path, width, height, mimeType);
			// [BUGFIX]-MOD by TCTNB,tao-li, 2014-07-21,PR735438 End
		} else {
			return null;
		}
	}
    ...
    public static ContentValues getContentValuesForData(String title,
    	...
    	Log.d(TAG,"###YUHUAN###getContentValuesForData#title=" + title);
...
//[FEATURE]-Add END by TCTNB,xutao.wu, 2014-07-24, PR635962
    public static long getAvailableSpace() {
        if (isSaveSDCard() && SDCard.instance().isWriteable()) {
            ...
            try {
                ...
                long ret = stat.getAvailableBlocksLong() * (long) stat.getBlockSizeLong();
                return ret;
            } catch (Exception e) {
            }
            return UNKNOWN_SIZE;
        } else if (isSaveSDCard() && !SDCard.instance().isWriteable()) {
            // the default storage was sdcard, change it to phone storage
            /// modify by yaping.liu for pr966435 {
            setSaveSDCard(false, false);
            /// }
    ...
    private static Uri insertImage(ContentResolver resolver, ContentValues values) {
    	Log.d(TAG,"###YUHUAN###insertImage into ");
        Uri uri = null;
        try {
