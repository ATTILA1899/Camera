...
public class FocusOverlayManager {
    private static final String TAG = "YUHUAN_FocusOverlayManager";
    ...
    public static final int STATE_CONTINUOUS_FOCUSING=5;
    public static final int STATE_CONTINUOUS_FOCUSING_SNAP_ON_FINISH=6;
    ...
    public FocusOverlayManager(ComboPreferences preferences, String[] defaultFocusModes,
    	...
    	CameraActivity.TraceLog(TAG, "FocusOverlayManager Constructor",new Throwable());
    ...
    public boolean isFocusIdle(){
    	if(mState==STATE_IDLE||mState==STATE_SUCCESS||mState==STATE_FAIL){
    		return true;
    	}else{
    		return false;
    	}
    ...
    private void unlockAeAwbIfNeeded() {
        if (mLockAeAwbNeeded && mAeAwbLock && (mState != STATE_FOCUSING_SNAP_ON_FINISH)
        		&&(mState!=STATE_CONTINUOUS_FOCUSING_SNAP_ON_FINISH)) {
            mAeAwbLock = false;
            mListener.setFocusParameters();
        }
    }
    ...
    public void doSnapInLongshot(){
    	 if (!mInitialized) return;

         CameraActivity.TraceLog(TAG, "do snap in longshot");
    ...
    public void doSnap() {
        ...
        CameraActivity.TraceLog(TAG, "###YUHUAN###doSnap ");
        // If the user has half-pressed the shutter and focus is completed, we
        // can take the photo right away. If the focus mode is infinity, we can
        // also take the photo.
        if (!needAutoFocusCall() || (mState == STATE_SUCCESS || mState == STATE_FAIL)) {
        	CameraActivity.TraceLog(TAG, "TAF Finish , Start Capture");
            capture();
        } else if (mState == STATE_FOCUSING) {
            // Half pressing the shutter (i.e. the focus button event) will
            // already have requested AF for us, so just request capture on
            // focus here.
        	CameraActivity.TraceLog(TAG, "Under TAF");
            mState = STATE_FOCUSING_SNAP_ON_FINISH;
        }else if(mState==STATE_CONTINUOUS_FOCUSING){
        	CameraActivity.TraceLog(TAG, "Under CAF");
        	mState=STATE_CONTINUOUS_FOCUSING_SNAP_ON_FINISH;
        	...
        	CameraActivity.TraceLog(TAG, "State Idle , capture");
            capture();
        }
    }
    ...
    public void onAutoFocus(boolean focused, boolean shutterButtonPressed) {
        if (mState == STATE_FOCUSING_SNAP_ON_FINISH) {
            ...
            updateFocusUI();
            // modify by minghui.hua for PR945838,PR931315-sichao.hu
            CameraActivity.TraceLog(TAG, "TAF Finish , take picture");
            capture();
        } else if (mState == STATE_FOCUSING) {
    ...
    public void onAutoFocusMoving(boolean moving) {
        ...
        if (mUI.hasFaces()) {
        	CameraActivity.TraceLog(TAG, "Has face");
            mUI.clearFocus();
            if (mIsAFRunning) {
            	CameraActivity.TraceLog(TAG, "Need stop af running in case of FaceDetection");
                mUI.onFocusSucceeded(true);
                mIsAFRunning = false;
                if(mState==STATE_CONTINUOUS_FOCUSING_SNAP_ON_FINISH){
                	CameraActivity.TraceLog(TAG, "CAF finish , take picture");
                	capture();
                }
                mState=STATE_IDLE;
                mListener.checkShutter();
            }
            return;
        }
        CameraActivity.TraceLog(TAG, "mState is "+mState);

        // Ignore if we have requested autofocus. This method only handles
        // continuous autofocus.
        if (mState ==STATE_FOCUSING||mState==STATE_SUCCESS||mState==STATE_FAIL||mState==STATE_FOCUSING_SNAP_ON_FINISH) return;

        // animate on false->true trasition only b/8219520
        if (moving && !mPreviousMoving) {
        	mState=STATE_CONTINUOUS_FOCUSING;
            mUI.onFocusStarted();
            mIsAFRunning = true;
        } else if (!moving) {
            mUI.onFocusSucceeded(true);
    ...
    public void onSingleTapUp(int x, int y) {
        if (!mInitialized || mState == STATE_FOCUSING_SNAP_ON_FINISH||mState==STATE_CONTINUOUS_FOCUSING_SNAP_ON_FINISH) return;
        ...
        if(mUI != null){
            mUI.updateFocusAreaLayout(mPreviewRect.width(), mPreviewRect.height(), x, y);
        }
    ...
    //PR930301-sichao.hu add begin
    public void onSingleTapForTouchAE(int x ,int y){
    	if (!mInitialized || mState == STATE_FOCUSING_SNAP_ON_FINISH||mState==STATE_CONTINUOUS_FOCUSING_SNAP_ON_FINISH) return;
    ...
    public String getFocusMode() {
        ...
        if (mFocusAreaSupported && !mFocusDefault) {
            ...
            mFocusMode = mPreferences.getString(
                    CameraSettings.KEY_FOCUS_MODE, Parameters.FOCUS_MODE_CONTINUOUS_PICTURE);
...
//            initializeFocusAreas(mPreviewRect.centerX(), mPreviewRect.centerY());
    ...
    public boolean isFocusingSnapOnFinish() {
        return mState == STATE_FOCUSING_SNAP_ON_FINISH||mState==STATE_CONTINUOUS_FOCUSING_SNAP_ON_FINISH;
    }
