...
public class IconListPreference extends ListPreference {
    ...
    public IconListPreference(Context context, AttributeSet attrs) {
        ...
        mHighlightIconIds = getIds(res, a.getResourceId(
                R.styleable.IconListPreference_highlighticons, 0));
        a.recycle();
    }
    ...
    public int[] getHighlightIconIds() {
        return mHighlightIconIds;
    }
    ...
    public void setHighLightIconIds(int[] iconIds) {
        mHighlightIconIds = iconIds;
    }
    ...
    public void filterUnsupported(List<String> supported) {
        ...
        IntArray highlighticonIds = new IntArray();
        ...
        for (int i = 0, len = entryValues.length; i < len; i++) {
            if (supported.indexOf(entryValues[i].toString()) >= 0) {
                ...
                if (mHighlightIconIds != null) highlighticonIds.add(mHighlightIconIds[i]);
            }
        }
        if (mIconIds != null) mIconIds = iconIds.toArray(new int[iconIds.size()]);
        if (mHighlightIconIds != null) mHighlightIconIds = highlighticonIds.toArray(new int[highlighticonIds.size()]);
