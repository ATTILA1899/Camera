...
//    private ResourceTranscoder<Bitmap,Bitmap> coder;
...
//        if(this instanceof PhotoData && ((PhotoData)this).getOrientation()!=0){
...
//        }
        ...
        try{
            ...
            Log.e(TAG, "Glide load failed in LocalMediaData.fillImageView");
        }
        return v;
    }
    ...
    public void loadFullImage(Context context, int thumbWidth, int thumbHeight, View v,
            LocalDataAdapter adapter){
        //Just for implement in PhotoData to load full size image
    }
    ...
    public static final class PhotoData extends LocalMediaData {
        ...
        protected ImageView fillImageView(Context ctx, ImageView v,
                int decodeWidth, int decodeHeight, Drawable placeHolder,
                LocalDataAdapter adapter) {
            if(getOrientation()!=0){//The exif rotation support of Glide is not that good , need to roll back to original image reader in case of rotated pictures
                v.setScaleType(ImageView.ScaleType.FIT_XY);
                v.setImageDrawable(placeHolder);
                ...
                loadImage(ctx, v, decodeWidth, decodeHeight, placeHolder, false);
            }
        ...
        public void loadFullImage(Context context, int thumbWidth, int thumbHeight, View v,
            ...
            if(v instanceof ImageView){
                loadImage(context, (ImageView) v, thumbWidth, thumbHeight, null, true);
            }
        }
        
        
        //PR1005622-sichao.hu add begin . The glide will do decode job when the given size picture is not decoded yet .  
        //Once the full image thumb size is not the same as normal image final result , the glide will reset the image content into the imageView
        //We ensure that the decode size will keep consistent for both cases
        private int mDecodeWidth=0;
        private int mDecodeHeight=0;
        
        private boolean isLandscape(){
            return mOrientation%180!=0;
        }
        //PR1005622-sichao.hu add end
        private void loadImage(Context context, ImageView imageView, int thumbWidth,
                int thumbHeight, Drawable placeHolder, boolean full) {

                mView=imageView;
                //TODO: Figure out why these can be <= 0.
                /*Add signature here to make sure every time the photo in mediastore edited , the load wouldn't reuse the cache identified by modifiedDate*/
                MediaStoreSignature ms=new MediaStoreSignature(mMimeType,mDateModifiedInSeconds,mOrientation);//PR1025072-sichao.hu added
                if (thumbWidth <= 0 || thumbHeight <=0) {
                    return;
                }
                try{
                    BitmapRequestBuilder<Uri, Bitmap> request = Glide.with(context)
                        .loadFromMediaStore(getContentUri())
                        .asBitmap().signature(ms)////PR1025072-sichao.hu added
                    ...
                    if (full) {
                        if(mDecodeWidth==0){
                            mDecodeWidth=thumbWidth;
                            mDecodeHeight=thumbHeight;
                        }
                        request.thumbnail(Glide.with(context)
                                .loadFromMediaStore(getContentUri())
                                .asBitmap().signature(ms)//PR1025072-sichao.hu added
                                .encoder(JPEG_ENCODER)
                                .override(mDecodeWidth, mDecodeHeight)
                                .fitCenter())
                        ...
                        request.thumbnail(Glide.with(context)
                                .loadFromMediaStore(getContentUri())
                                .asBitmap().signature(ms)
                                .encoder(JPEG_ENCODER)
                                .override(MEDIASTORE_THUMB_WIDTH, MEDIASTORE_THUMB_HEIGHT))
                            .override(mDecodeWidth, mDecodeHeight);//this image content will be the thumb for full image decoding
                    }
                    request.into(imageView);
                }catch(Exception e){//In case of to request a load when this activity is destroyed
                    Log.w(TAG, "This activity is destroyed, ignore this case");
                }
        }
        ...
        private final class PhotoBitmapLoadTask extends BitmapLoadTask {
            ...
            protected Bitmap doInBackground(Void... v) {
                ...
                Bitmap tmpBitmap = BitmapFactory.decodeFile(mPath, justBoundsOpts);
...
//                Bitmap b = BitmapFactory.decodeFile(mPath);
                ...
                if (mOrientation != 0 && b != null) {
                    ...
                    Bitmap tmp= Bitmap.createBitmap(b, 0, 0, b.getWidth(), b.getHeight(), m, false);
                    if(tmp!=b){
                    	b.recycle();
                    }
                    b=tmp;
